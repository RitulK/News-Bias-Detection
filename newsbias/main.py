    ...
    async def _find_matching_event(self, embedding: np.ndarray) -> ObjectId:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get('http://localhost:8080/eventembeddings', timeout=5) as response:
    ...
    async def _assign_article_to_event(self, article: Dict, event_id: ObjectId, embedding: np.ndarray):
        ...
        try:
            response = await aiohttp.ClientSession().get(
                f'http://localhost:8080/eventData/{event_id}',
                timeout=aiohttp.ClientTimeout(total=5)
            )
        ...
        if event_embeddings:
            ...
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    await session.put(
                        f'http://localhost:8080/event/{event_id}',
                        json=update_payload,
                        timeout=aiohttp.ClientTimeout(total=5)
                    )
            except:
